...
class Solver:
    ...
    def __init__(self, layer_stack: typing.List[Layer], grating_period: float, wavelength: float, n_harmonics: int = 0, theta: float = 0):
        ...
        for i, layer in enumerate(self.layer_stack[1:]):
            ...
            ff.printdf(np.linalg.inv(trns_matrix))
...
#             ff.printdf(phase_matrix)
#             inv_phase_matrix = np.linalg.inv(phase_matrix)
#             ff.printdf(inv_phase_matrix)
...
#             ff.printdf(fwd_phase_matrix)
#             ff.printdf(bwd_phase_matrix)
...
#             m11, m12, m21, m22 = ff.quar(three_layer_mat)
...
#             m11, m12, m21, m22 = ff.quar(master_matrix)
...
#         magnetic_field_harmonics = ff.manual_matmul(
#             q, electric_field_harmonics, np.linalg.inv(np.diag(kz_components)))

#         # magnetic_field_harmonics = ff.manual_matmul(ff.manual_matmul(
#         #     q, electric_field_harmonics), np.linalg.inv(np.diag(kz_components*1j)))

#         # magnetic_field_harmonics = ff.manual_matmul(q, electric_field_harmonics)
